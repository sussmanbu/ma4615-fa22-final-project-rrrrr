...
MA_select2 <- MA_select %>% 
  filter(!is.na(`Weather Condition`) & !is.na(Temperature) & !is.na(Visibility), Temperature <= 104)
...
Combined %>% 
  ggplot(aes(MY, Avg_traffic_volume, color = `Weather Condition`)) + geom_point() + geom_line(aes(group = `Weather Condition`)) 

Combined %>% 
  filter(`Weather Condition` == "Clear") %>% 
  ggplot(aes(MY, Avg_traffic_volume)) + geom_point() + geom_line(aes(group = 1)) 
...
Combined %>% 
  ...
  mutate(count = n()) %>% 
  ungroup() %>% 
  ggplot(aes(MY, count)) + geom_point() + geom_line(aes(group = 1)) 
```
...
(
  MA_reg <- lm(Accident_Count ~ `Train Speed` + `Estimated Vehicle Speed`, data = MA_regression) %>%
    broom:: tidy() %>% 
